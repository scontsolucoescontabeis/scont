...
// SUPABASE_URL e SUPABASE_KEY são carregados de ../supabase-config.js via index.html
let supabaseClient;
...
// Adicionar após a inicialização do Supabase
const REQUIRED_FIELDS = {
  formularios: [
    'id', 'tipo', 'status', 'nome_empresa', 'cnpj', 'email', 
    'telefone', 'responsavel', 'observacoes', 'data_criacao', 
    'data_atualizacao', 'endereco', 'cidade', 'estado', 'cep'
  ],
  empregados: [
    'id', 'tipo', 'status', 'nome_empregado', 'cpf', 'email', 
    'telefone', 'responsavel', 'observacoes', 'data_criacao', 
    'data_atualizacao', 'empresa_id'
  ],
  socios: [
    'id', 'formulario_id', 'nome', 'profissao', 'participacao', 
    'cargo', 'data_criacao'
  ]
};

function validateAndNormalizeData(data, tableType) {
  if (!data) return null;
  
  const required = REQUIRED_FIELDS[tableType] || [];
  const missingFields = required.filter(field => !(field in data));
  
  if (missingFields.length > 0) {
    console.warn(`Campos faltantes em ${tableType}:`, missingFields);
                ...
                ${sociosDoBanco.map(socio => `
                    <div style="background: #f8f9fa; border: 1px solid var(--border-color); border-radius: var(--radius-md); padding: 15px; margin-bottom: 15px;">
                        ...
                        <div class="detail-row">
                            <div class="detail-field">
                                <span class="detail-label">CPF</span>
                                <div class="detail-value" style="background: white;">${sanitizeOutput(socio.cpf || '-')}</div>
                            </div>
                            <div class="detail-field">
                                <span class="detail-label">RG</span>
                                <div class="detail-value" style="background: white;">${sanitizeOutput(socio.rg || '-')}</div>
                            </div>
                        </div>
                        <div class="detail-row">
                            <div class="detail-field">
                                <span class="detail-label">Endereço Residencial</span>
                                <div class="detail-value" style="background: white;">${sanitizeOutput(socio.endereco_residencial || '-')}</div>
                            </div>
                            <div class="detail-field">
                                <span class="detail-label">CEP</span>
                    ...
                    <div style="background: #f8f9fa; border: 1px solid var(--border-color); border-radius: var(--radius-md); padding: 15px; margin-bottom: 15px;">
                        ...
                        <div class="detail-row full">
                            <div class="detail-field">
                                ...
                                <input type="text" id="editSocioEndereco_${socio.id}" class="search-input" style="padding: var(--spacing-md);" value="${sanitizeOutput(socio.endereco_residencial || '')}">
                            </div>
                        </div>
                    </div>
                `).join('')}
            ...
            for (const socio of window.currentSocios) {
                ...
                if (document.getElementById(`editSocioNome_${socioId}`)) {
                    const socioUpdateData = {
                        ...
                        endereco_residencial: document.getElementById(`editSocioEndereco_${socioId}`).value
                    };
...
async function viewDocuments(formId) {
    try {
        ...
        if (tipo === 'registro') {
            ...
            const base = `Novos-Empregados/${sanitizeFileName(nomeEmpresa)}/${sanitizeFileName(nomeEmpregado)}/`;
            caminhos = [
                base + 'Formulario/',
                base + 'Documentos-de-Identificacao/',
                base + 'Documentos-Familiares/',
                base + 'Documentos-Medicos/'
            ];
        }
...
async function loadHistory() {
    try {
        ...
        const { data: history, error } = await supabaseClient.from('historico_edicoes').select('*').order('editado_em', { ascending: false });
        ...
        tableBody.innerHTML = history.map(item => `
            <tr>
                <td>${new Date(item.editado_em).toLocaleString('pt-BR')}</td>
                <td>${sanitizeOutput(item.editado_por)}</td>
                <td>${sanitizeOutput(item.tabela)} / ID: ${item.registro_id}</td>
                <td><strong>${sanitizeOutput(item.campo)}</strong></td>
...
function filterHistory() {
    ...
    const filtered = window.allHistory.filter(item => {
        const matchSearch = !search || (item.editado_por && item.editado_por.toLowerCase().includes(search)) || (item.campo && item.campo.toLowerCase().includes(search));
        const matchDate = !date || new Date(item.editado_em).toISOString().split('T')[0] === date;
...
function sanitizeFileName(name) {
    return String(name).normalize('NFD').replace(/[̀-ͯ]/g, '').toLowerCase().replace(/[^a-z0-9]/g, '_').replace(/_+/g, '_').replace(/^_+|_+$/g, '');
}
